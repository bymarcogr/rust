pub mod english {
    pub const APP_TITLE: &str = "Fast File Flow - Unir";
    pub const SEARCH_PLACEHOLDER: &str = "Search";

    pub const ERROR_LOAD_ICON: &str = "Failed to load icon";
    pub const ERROR_GET_FOLDER: &str = "Failed to get current folder";
}

pub mod path {
    pub const ICON_PATH: &str = "resources/images/icon.png";
    pub const LOGO_PRIMARY_PATH: &str = "\\src\\resources\\images\\logo-black.png";
    pub const LOGO_SECONDARY_PATH: &str = "\\src\\resources\\images\\logo-white.png";
}

pub mod sizes {
    pub const APP_HEIGHT: f32 = 860.0;
    pub const APP_WIDTH: f32 = 1600.0;
    pub const MENU_BUTTON_HEIGHT: f32 = 40.0;
    pub const MENU_BUTTON_WIDTH: f32 = 40.0;
    pub const PANEL_HEIGHT: f32 = 200.0;
    pub const PANEL_WIDTH: f32 = 350.0;
}

pub mod icons {
    pub const SEARCH: char = '\u{E800}';
    pub const USER: char = '\u{E801}';
    pub const MENU: char = '\u{F0C9}';
    pub const REFRESH: char = '\u{E802}';
    pub const FILTER: char = '\u{F0B0}';
    pub const PROCESS: char = '\u{F2DB}';
    pub const ADD: char = '\u{E803}';
    pub const SCRIPT: char = '\u{F121}';
    pub const ANALYSIS: char = '\u{E804}';
    pub const PIPELINE: char = '\u{E805}';
    pub const AI: char = '\u{F0D0}';
    pub const PREVIEW: char = '\u{E806}';
    pub const SAVE: char = '\u{E807}';
    pub const EXPORT: char = '\u{E808}';

    pub const TAB_SPACE: &str = "   ";
}
